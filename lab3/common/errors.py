...
class VersionMismatchError(Exception):
    ...
    def __str__(self):
        return (
            "expected update version ({}) does not match received version ({})".format(
...
class UnknownUserError(Exception):
    ...
    def __str__(self):
        return "unknown user {}".format(self.username)
...
class InvalidPublicProfile(Exception):
    ...
    def __str__(self):
        return "No valid public profile could be recovered for {}".format(
            self.name_friend
        )
...
class AlbumOwnerError(Exception):
    ...
    def __str__(self):
        return "Only the owner of {} can change the sharing preference".format(
            self.name_album
        )


class AlbumPermissionError(Exception):
    def __init__(self, name_album=None):
        self.name_album = name_album

    def __str__(self):
        return "You don't have the permissions to access the album {}".format(
            self.name_album
        )
