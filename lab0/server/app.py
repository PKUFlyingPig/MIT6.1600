...
def map_rpc_type(server: Server, rpc_type: str):
    ...
    RPC_REQUEST_MAPPING: t.Dict[str, t.Tuple[types.RpcObject, t.Callable[[types.RpcObject], types.RpcObject]]] = {
        "LoginRequest": (types.LoginRequest, server.login),
        "RegisterRequest": (types.RegisterRequest, server.register_user),
        "UpdatePublicProfileRequest": (types.UpdatePublicProfileRequest, server.update_public_profile),
        "GetFriendPublicProfileRequest": (types.GetFriendPublicProfileRequest, server.get_friend_public_profile),
